...
void Anchor::ProcessIncomingPacketQueue() {
    if (packetsQueuedFromTeamState) {
        if (IsSaveLoaded()) {
            packetsQueuedFromTeamState = false;
        } else {
            return;
        }
    }
...
void Anchor::InitializeMultiWorld(std::set<std::string>& teams) {
    ...
    for (int i = 0; i < roomState.teams.size(); i++) {
        ...
        for (auto& [randoCheckId, randoStaticCheck] : Rando::StaticData::Checks) {
            if (RANDO_SAVE_CHECKS[randoCheckId].shuffled && !RANDO_SAVE_CHECKS[randoCheckId].skipped) {
...
void Anchor::ReleaseWorldForCurrentTeam() {
    if (!IsSaveLoaded() || roomState.teams.size() < 2) {
        return;
    }

    std::string currentTeamId = CVarGetString("gNetwork.Anchor.TeamId", "default");
    s16 currentTeamIndex = -1;
    for (size_t i = 0; i < roomState.teams.size(); i++) {
        if (roomState.teams[i] == currentTeamId) {
            currentTeamIndex = i;
            break;
            ;
        }
    ...
    for (int i = 0; i < RC_MAX; i++) {
        auto& randoSaveCheck = RANDO_SAVE_CHECKS[i];

        if (!randoSaveCheck.obtained && randoSaveCheck.multiWorldTeamIndex != currentTeamIndex) {
            RandoItemId randoItemId = Rando::ConvertItem(RANDO_SAVE_CHECKS[i].randoItemId, (RandoCheckId)i);
            randoSaveCheck.cycleObtained = true;
            randoSaveCheck.obtained = true;
            randoSaveCheck.eligible = false;
            Anchor::Instance->SendPacket_SetCheckStatus((RandoCheckId)i);
