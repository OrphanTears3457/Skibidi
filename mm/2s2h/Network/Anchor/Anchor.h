...
typedef struct {
    ...
    std::vector<std::string> teams; // List of teams in multiworld, used for determining multiWorldTeamIndex per check
} RoomState;
...
class Anchor : public Network {
  private:
    ...
    bool packetsQueuedFromTeamState = false;
  ...
  public:
    ...
    void InitializeMultiWorld(std::set<std::string>& teams);
    void ReleaseWorldForCurrentTeam();
