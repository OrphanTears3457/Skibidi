...
void AnchorMainMenu(WidgetInfo& info) {
    ...
    if ((anchor->roomState.allowEveryoneToRelease || anchor->roomState.ownerClientId == anchor->ownClientId) &&
        !anchor->roomState.teams.empty()) {
        if (UIWidgets::Button("Release World For Current Team",
                              UIWidgets::ButtonOptions()
                                  .Color(UIWidgets::Colors::Red)
                                  .Tooltip("Releases the current multi-world team from the "
                                           "room, allowing them to continue playing solo."))) {
            Anchor::Instance->ReleaseWorldForCurrentTeam();
        }
    }
}

static std::set<std::string> teamsForMwInit;
void AnchorAdminMenu(WidgetInfo& info) {
    ...
    if (UIWidgets::CVarCheckbox("Allow Everyone To Release (MW)", "gNetwork.Anchor.RoomSettings.AllowEveryoneToRelease",
    ...
    if (Anchor::Instance->roomState.teams.empty()) {
        std::string teamToRemove;
        ImGui::AlignTextToFramePadding();
        ImGui::Text("Teams:");
        ImGui::SameLine();
        if (UIWidgets::Button(ICON_FA_PLUS, { .size = UIWidgets::Sizes::Inline, .color = UIWidgets::Colors::Green })) {
            ImGui::OpenPopup("TeamPopup");
        }
        for (auto& team : teamsForMwInit) {
            ImGui::AlignTextToFramePadding();
            ImGui::Text("%s", team.c_str());
            ImGui::SameLine();
            if (UIWidgets::Button(ICON_FA_TIMES,
                                  { .size = UIWidgets::Sizes::Inline, .color = UIWidgets::Colors::Red })) {
                teamToRemove = team;
            }
        }
        if (!teamToRemove.empty()) {
            teamsForMwInit.erase(teamToRemove);
            teamToRemove.clear();
        }
        if (ImGui::BeginPopup("TeamPopup")) {
            ...
            for (auto& spoilerFile : Rando::Spoiler::spoilerOptions) {
                ...
                if (spoilerFile == "Generate New Seed" || teamsForMwInit.contains(teamName)) {
                    continue;
                }

                if (ImGui::Selectable(teamName.c_str())) {
                    teamsForMwInit.insert(teamName);
                }
            }
            ImGui::EndPopup();
        }
        bool disabled = teamsForMwInit.size() < 2 || Anchor::Instance->IsSaveLoaded();
        if (disabled) {
            ImGui::BeginDisabled();
        }
        if (UIWidgets::Button("Initialize Multi-World")) {
            Anchor::Instance->InitializeMultiWorld(teamsForMwInit);
        }
        if (disabled) {
            ImGui::EndDisabled();
        }
        ...
        ImGui::Text("Teams:");
        for (auto& team : Anchor::Instance->roomState.teams) {
            ImGui::Text("%s", team.c_str());
        }
    }
}
...
void AnchorInstructionsMenu(WidgetInfo& info) {
    ...
    ImGui::TextWrapped(
        ...
        "supports no-logic, this will be the case for the forseeable future.");
    ...
    ImGui::SeparatorText("Multiworld Usage Instructions");

    ImGui::TextWrapped("1. The host should prepare for the multiworld by creating spoiler files for each team "
                       "involved, ensuring that the name of the spoiler file matches the Team ID.");

    ImGui::TextWrapped("2. The host player should set up a room by entering a Room ID and the Team ID they belong to, "
                       "then enable Anchor.");

    ImGui::TextWrapped("3. Configure the room settings as desired (PvP, teleporting, etc).");

    ImGui::TextWrapped("4. The host should then initialize the multiworld by selecting the Spoiler files for the "
                       "associated teams from the dropdown and clicking 'Initialize Multi-World'.");

    ImGui::TextWrapped(
        "5. Other players should then enter the same Room ID and their associated Team IDs and enable Anchor.");

    ImGui::TextWrapped("6. All players should then each create a new randomizer file (the configuration does not "
                       "matter, it will be overridden by multi-world), and load into the game.");

    ImGui::TextWrapped("7. All players should now be connected and able to see each other in-game, occasionally you "
                       "may need to leave and re-enter South clock town if a player does not appear.");
}
